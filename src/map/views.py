...
def showmap(request):
    fund_list_103 = FundGet.objects.filter(year=103).exclude(
        city=23
    ).values('city').annotate(Sum('money'))
    ...
    surp_list_103 = Surplus.objects.filter(
        year=103
    ).values('city').annotate(Sum('surplus'))
    ...
    organization_list = json.dumps(
        [
            v
            for v in organization_list.values(
    ...
    location_list = json.dumps(
        [
            v
            for v in location_list.values(
                'id', 'name', 'center_longitude', 'center_latitude',
                'ne_longitude', 'ne_latitude', 'sw_longitude', 'sw_latitude'
            )
        ]
    )
    return render(
